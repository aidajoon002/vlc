...
NS_LOG_COMPONENT_DEFINE ("ApVlcMac");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (ApVlcMac)
  ;

TypeId
ApVlcMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ApVlcMac")
    .SetParent<RegularWifiMac> ()
    .AddConstructor<ApVlcMac> ()
    .AddAttribute ("BeaconInterval", "Delay between two beacons",
                   TimeValue (MicroSeconds (102400)),
                   MakeTimeAccessor (&ApVlcMac::GetBeaconInterval,
                                     &ApVlcMac::SetBeaconInterval),
                   MakeTimeChecker ())
    .AddAttribute ("BeaconJitter", "A uniform random variable to cause the initial beacon starting time (after simulation time 0) to be distributed between 0 and the BeaconInterval.",
                   ...
                   MakePointerAccessor (&ApVlcMac::m_beaconJitter),
                   MakePointerChecker<UniformRandomVariable> ())
    .AddAttribute ("EnableBeaconJitter", "If beacons are enabled, whether to jitter the initial send event.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&ApVlcMac::m_enableBeaconJitter),
                   MakeBooleanChecker ())
    .AddAttribute ("BeaconGeneration", "Whether or not beacons are generated.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&ApVlcMac::SetBeaconGeneration,
                                        &ApVlcMac::GetBeaconGeneration),
                   MakeBooleanChecker ())
  ;
  return tid;
}
...
void
ApVlcMac::SetAddress (Mac48Address address)
{
...
void
ApVlcMac::SetBeaconGeneration (bool enable)
{
  ...
  if (!enable)
    ...
    {
      m_beaconEvent = Simulator::ScheduleNow (&ApVlcMac::SendOneBeacon, this);
    }
  m_enableBeaconGeneration = enable;
}

bool
ApVlcMac::GetBeaconGeneration (void) const
{
  NS_LOG_FUNCTION (this);
...
Time
ApVlcMac::GetBeaconInterval (void) const
{
  NS_LOG_FUNCTION (this);
...
void
ApVlcMac::SetWifiRemoteStationManager (Ptr<WifiRemoteStationManager> stationManager)
{
...
void
ApVlcMac::SetLinkUpCallback (Callback<void> linkUp)
{
...
void
ApVlcMac::SetBeaconInterval (Time interval)
{
...
int64_t
ApVlcMac::AssignStreams (int64_t stream)
{
...
void
ApVlcMac::ForwardDown (Ptr<const Packet> packet_vlc, Mac48Address from,
                        Mac48Address to)
{
  NS_LOG_FUNCTION (this << packet_vlc << from << to);
  ...
  if (m_qosSupported)
    {
      tid = QosUtilsGetTidForPacket (packet_vlc);
  ...
  ForwardDown (packet_vlc, from, to, tid);
}
...
{
  NS_LOG_FUNCTION (this << packet_vlc << from << to << static_cast<uint32_t> (tid));
  WifiMacHeader hdr;
  ...
  if (m_qosSupported)
    {
      ...
      NS_ASSERT (tid < 8);
      m_edca[QosUtilsMapTidToAc (tid)]->Queue (packet_vlc, hdr);
    }
  else
    {
      m_dca->Queue (packet_vlc, hdr);
    }
}

void
ApVlcMac::Enqueue (Ptr<const Packet> packet_vlc, Mac48Address to, Mac48Address from)
{
  NS_LOG_FUNCTION (this << packet_vlc << to << from);
  if (to.IsBroadcast () || m_stationManager->IsAssociated (to))
    {
      ForwardDown (packet_vlc, from, to);
    }
}

void
ApVlcMac::Enqueue (Ptr<const Packet> packet_vlc, Mac48Address to)
{
  NS_LOG_FUNCTION (this << packet_vlc << to);
  // We're sending this packet with a from address that is our own. We
  // get that address from the lower MAC and make use of the
  // from-spoofing Enqueue() method to avoid duplicated code.
  Enqueue (packet_vlc, to, m_low->GetAddress ());
}

bool
ApVlcMac::SupportsSendFrom (void) const
{
  NS_LOG_FUNCTION (this);
  return true;
}
...
SupportedRates
ApVlcMac::GetSupportedRates (void) const
{
  NS_LOG_FUNCTION (this);
...
HtCapabilities
ApVlcMac::GetHtCapabilities (void) const
{
...
void
ApVlcMac::SendProbeResp (Mac48Address to)
{
  ...
  hdr.SetDsNotFrom ();
  hdr.SetDsNotTo ();
  Ptr<Packet> packet_vlc = Create<Packet> ();
...
void
ApVlcMac::SendAssocResp (Mac48Address to, bool success)
{
...
ApVlcMac::SendOneBeacon (void)
{
  ...
  m_beaconEvent = Simulator::Schedule (m_beaconInterval, &ApVlcMac::SendOneBeacon, this);
}

void
ApVlcMac::TxOk (const WifiMacHeader &hdr)
{
  NS_LOG_FUNCTION (this);
...
void
ApVlcMac::TxFailed (const WifiMacHeader &hdr)
{
  NS_LOG_FUNCTION (this);
...
void
ApVlcMac::Receive (Ptr<Packet> packet_vlc, const WifiMacHeader *hdr)
{
  NS_LOG_FUNCTION (this << packet_vlc << hdr);
          ...
          && m_stationManager->IsAssociated (from))
        {
          ...
          if (to == GetAddress ())
              ...
              if (hdr->IsQosData ())
                {
                  if (hdr->IsQosAmsdu ())
                    {
                      NS_LOG_DEBUG ("Received A-MSDU from=" << from << ", size=" << packet_vlc->GetSize ());
                      DeaggregateAmsduAndForward (packet_vlc, hdr);
                      packet_vlc = 0;
                    }
                  else
                    {
                      ForwardUp (packet_vlc, from, bssid);
                    }
                }
              else
                {
                   ...
                   || m_stationManager->IsAssociated (to))
            {
              ...
              if (hdr->IsQosData ())
                {
                  ForwardDown (packet_vlc, from, to, hdr->GetQosTid ());
                }
              else
                {
                  ForwardDown (packet_vlc, from, to);
          ...
          if (hdr->IsAssocReq ())
            {
              ...
              packet_vlc->RemoveHeader (assocReq);
  ...
  RegularWifiMac::Receive (packet_vlc, hdr);
}

void
ApVlcMac::DeaggregateAmsduAndForward (Ptr<Packet> aggregatedPacket,
      ...
      if (m_enableBeaconJitter)
        {
          ...
          m_beaconEvent = Simulator::Schedule (MicroSeconds (jitter), &ApVlcMac::SendOneBeacon, this);
        }
      else
        {
